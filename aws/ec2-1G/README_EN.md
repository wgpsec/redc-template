...
This scene deploys a lightweight ARM64 EC2 host in AWS ap-east-1 (Hong Kong), using `t4g.micro` and a Debian image by default. It is suitable for a low-cost jump host, a lightweight test machine, or a minimal base node.
...
- The default instance type is the ARM64 `t4g.micro`. If you override `instance_type`, make sure the selected `ami` uses a compatible architecture.
...
| `instance_type` | `t4g.micro` | EC2 instance type. The current default is a lightweight ARM64 family. |
